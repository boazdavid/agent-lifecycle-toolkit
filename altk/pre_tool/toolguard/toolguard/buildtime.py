...
async def build_toolguards(
	...
	if oas_path: # for cases 1 and 2
		...
		return await generate_guards_from_specs(oas_path, tool_policies, step2_out_dir, llm_data=step2_llm, app_name=app_name, lib_names=None, tool_names=tools2run)
	...
	if isinstance(tools, list) and not oas_path:
		...
		tool_policies = await extract_toolguard_specs(policy_text, tools_info, step1_out_dir, step1_llm, tools2run, short1)
		return await generate_guards_from_specs(tools, tool_policies, step2_out_dir, llm_data=step2_llm, app_name=app_name, lib_names=None, tool_names=tools2run)
...
async def generate_guard_specs(
	...
	if isinstance(tools, list) and all([isinstance(tool, BaseTool) for tool in tools]):
		oas = langchain_tools_to_openapi(tools) # type: ignore
		oas_path = join(work_dir, "oas.json")
		oas.save(oas_path)
	...
	if oas_path: # for cases 1 and 2
		...
		summarizer = OASSummarizer(oas)
		tools_info = summarizer.summarize()
		return await extract_toolguard_specs(policy_text, tools_info, work_dir, llm, None, True)
...
async def generate_guards_from_specs(
	...
	if oas_path: # for cases 1 and 2
		return await generate_toolguards_from_openapi(app_name, tool_specs, work_dir, oas_path, llm_data)

	# Case 3: List of functions + case 4: List of methods
	funcs = [cast(Callable, tool) for tool in tools]
	return await generate_toolguards_from_functions(app_name, tool_specs, work_dir, funcs=funcs, llm_data=llm_data, module_roots=lib_names)
	

#load step1 results
def load_policies_in_folder(folder:str)->List[ToolGuardSpec]:
